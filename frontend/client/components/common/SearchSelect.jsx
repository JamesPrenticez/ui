...
function SearchSelect({data, selected, setSelected}) {
  const [filteredArray, setFilteredArray] = useState(data.map(item => item.name))
  ...
  useEffect(() => {
    setTimeout(() => {
      if(toggle && downArrowPressed) setCursor(prevState => prevState < data.length - 1 ? prevState + 1 : prevState)
    }, 100)
  }, [downArrowPressed, cursor])

  useEffect(() => {
    setTimeout(() => {
      if(toggle && upArrowPressed) setCursor(prevState => (prevState > 0 ? prevState - 1 : prevState))
    }, 100)
  ...
  const updateFilteredArray = (value) => {
    let filteredArr = data.filter(item => item.name.toLowerCase().includes(value.toLowerCase()))
    ...
    <>
      ...
      >
        ...
        <div className="absolute right-2 inset-y-0 pl-3 flex items-center  pointer-events-none">
          <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 text-gray-500 ${!toggle && "transfrom rotate-90"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
